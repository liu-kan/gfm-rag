...
def create_graph(
    ...
    try:
        ...
        if "hotpotqa" in dataset or "custom" in dataset:
            queries = json.load(open(f"data/{dataset}/raw/dataset.json"))
    ...
    if create_graph_flag:
        ...
        if cosine_sim_edges:
            if 'colbert' in retriever_name:
                kb_similarity = pickle.load(open('output/lm_vectors/colbert/{}/nearest_neighbor_kb_to_kb.p'.format(dataset), 'rb'))
            else:
                kb_similarity = pickle.load(open('output/lm_vectors/{}_mean/nearest_neighbor_kb_to_kb.p'.format(processed_retriever_name), 'rb'))
            ...
            pickle.dump(synonym_candidates, open(
                'data/{}/tmp/{}_similarity_edges_mean_{}_thresh_{}_{}_{}.{}.subset.p'.format(dataset, dataset, threshold, phrase_type, extraction_type, processed_retriever_name, version), 'wb'))
        else:
            graph_plus = graph

        pickle.dump(relations,
                    open('data/{}/tmp/{}_{}_graph_relation_dict_{}_{}_{}.{}.subset.p'.format(dataset, dataset, graph_type, phrase_type, extraction_type, processed_retriever_name, version), 'wb'))
        ...
        if similarity_max == 1.0:
            pickle.dump(graph_plus, open(
                'data/{}/tmp/{}_{}_graph_mean_{}_thresh_{}_{}_{}.{}.subset.p'.format(dataset, dataset, graph_type, threshold, phrase_type,
                                                                                extraction_type, processed_retriever_name, version), 'wb'))
        else:
            pickle.dump(graph_plus, open(
                'data/{}/tmp/{}_{}_graph_mean_{}_thresh_{}_{}_sim_max_{}_{}.{}.subset.p'.format(dataset, dataset, graph_type, threshold,
                                                                                           phrase_type, extraction_type, similarity_max, processed_retriever_name, version), 'wb'))

        json.dump(graph_json, open('data/{}/tmp/{}_{}_graph_chatgpt_openIE.{}_{}.{}.subset.json'.format(dataset, dataset, graph_type, phrase_type,
