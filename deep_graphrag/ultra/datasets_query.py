...
class LogicalQueryDataset(InMemoryDataset):
        ...
        for query_type in query_types:
            if "u" in query_type:
                if "-" not in query_type:
                    query_type = f"{query_type}-{self.union_type}"
    ...
    def process(self):
        ...
        for split in ["train", "valid", "test"]:
            triplet_file = os.path.join(path, f"{split}.txt")
        ...
        train_edges = torch.tensor(
            [[t[0], t[1]] for t in triplets[: num_samples[0]]], dtype=torch.long
        ).t()
        train_edge_types = torch.tensor(
            [t[2] for t in triplets[: num_samples[0]]], dtype=torch.long
        )
        ...
        for split in ["train", "valid", "test"]:
            pbar = tqdm(desc=f"Loading {split}-*.pkl", total=3)
            with open(os.path.join(path, f"{split}-queries.pkl"), "rb") as fin:
            ...
            if split == "train":
                with open(os.path.join(path, f"{split}-answers.pkl"), "rb") as fin:
                    query2easy_answers = pickle.load(fin)
                query2hard_answers = defaultdict(set)
                pbar.update(2)
            else:
                with open(os.path.join(path, f"{split}-easy-answers.pkl"), "rb") as fin:
                    query2easy_answers = pickle.load(fin)
                pbar.update(1)
                with open(os.path.join(path, f"{split}-hard-answers.pkl"), "rb") as fin:
                    query2hard_answers = pickle.load(fin)
                pbar.update(1)

            num_sample = sum([len(q) for t, q in type2queries.items()])
            pbar = tqdm(desc=f"Processing {split} queries", total=num_sample)
    ...
    def __getitem__(self, index):
        ...
        return {
            "query": F.pad(
                query, (0, self.max_query_length - len(query)), value=query.stop
            ),
...
class InductiveFB15k237Query(LogicalQueryDataset):
    ...
    def process(self):
        ...
        (
            ...
            vl_nodes,
            ts_nodes,
        ) = self.build_vocab(train_triplets, val_inference, test_inference)
        ...
        self.train_graph = Data(
            edge_index=torch.LongTensor(train_triplets)[:, :2].t(),
            edge_type=torch.LongTensor(train_triplets)[:, 2],
            num_nodes=len(tr_nodes),
            num_relations=num_relation,
        )

        # Validation graph: training triples (0..K) + new validation inference triples (K+1...K+M)
        self.valid_graph = Data(
            edge_index=torch.LongTensor(train_triplets + val_inference)[:, :2].t(),
        ...
        self.test_graph = Data(
            edge_index=torch.LongTensor(train_triplets + test_inference)[:, :2].t(),
        ...
        self.graph = Data(
            edge_index=torch.LongTensor(
                train_triplets + val_inference + test_inference
            )[:, :2].t(),
            edge_type=torch.LongTensor(train_triplets + val_inference + test_inference)[
                :, 2
            ],
            num_nodes=num_node,
            num_relations=num_relation,
    ...
    def load_queries(self, path):
        ...
        for split in ["train", "valid", "test"]:
            with open(os.path.join(path, f"{split}_queries.pkl"), "rb") as fin:
                struct2queries = pickle.load(fin)
            if split == "train":
                query2hard_answers = defaultdict(lambda: defaultdict(set))
                with open(os.path.join(path, f"{split}_answers_hard.pkl"), "rb") as fin:
                    query2easy_answers = pickle.load(fin)
            else:
                with open(os.path.join(path, f"{split}_answers_easy.pkl"), "rb") as fin:
                    query2easy_answers = pickle.load(fin)
                with open(os.path.join(path, f"{split}_answers_hard.pkl"), "rb") as fin:
                    query2hard_answers = pickle.load(fin)
            num_sample = 0
            structs = sorted(struct2queries.keys(), key=lambda s: self.struct2type[s])
            structs = tqdm(structs, f"Loading {split} queries")
            ...
            num_entity_for_sample += [
                getattr(self, f"{split}_graph").num_nodes
            ] * num_sample
            num_samples.append(num_sample)
...
class WikiTopicsQuery(InductiveFB15k237Query):
    ...
    def process(self):
        ...
        self.train_graph = Data(
            ...
            edge_type=torch.LongTensor(train_triplets)[:, 2],
            num_nodes=len(tr_nodes),
            num_relations=len(train_rel_vocab),
        )
        ...
        self.test_graph = Data(
            edge_index=torch.LongTensor(test_inference)[:, :2].t(),
            edge_type=torch.LongTensor(test_inference)[:, 2],
            num_nodes=len(ts_nodes),
            num_relations=len(test_rel_vocab),
        )
        if self.pre_transform:
            self.train_graph = self.pre_transform(self.train_graph)
            self.valid_graph = self.pre_transform(self.valid_graph)
        ...
        self.train_entity_vocab, self.inv_train_ent_vocab = (
            train_entity_vocab,
            {v: k for k, v in train_entity_vocab.items()},
        )
        self.train_relation_vocab, self.inv_train_rel_vocab = (
            train_rel_vocab,
            {v: k for k, v in train_rel_vocab.items()},
        )
        self.test_entity_vocab, self.inv_test_ent_vocab = (
            test_ent_vocab,
            {v: k for k, v in test_ent_vocab.items()},
        )
        self.test_relation_vocab, self.inv_test_rel_vocab = (
            test_rel_vocab,
            {v: k for k, v in test_rel_vocab.items()},
        )
    ...
    def load_queries(self, path):
        ...
        for split in ["train", "valid", "test"]:
            if split == "train":
                ...
                # load new answers
                with open(
                    os.path.join(path, f"train_answers_{split}.pkl"), "rb"
                ) as fin:
                    query2hard_answers = pickle.load(fin)
...
class JointDataset(LogicalQueryDataset):
    ...
    def __init__(self, path, graphs, query_types=None, union_type="DNF"):
        ...
        self.graphs = [
            self.datasets_map[dataset](
                path=path, query_types=query_types, union_type=union_type
            )
            for dataset in graphs.split(",")
        ]
        self.graph_names = graphs

        # Total number of samples obtained from iterating over all graphs
        self.num_samples = [
            sum(k) for k in zip(*[graph.num_samples for graph in self.graphs])
        ]
        self.valid_samples = [
            torch.cumsum(torch.tensor(k).flatten(), dim=0)
            for k in zip([graph.num_samples for graph in self.graphs])
        ]
